...
so that you can't access to a L<$writer> object, suitable for streaming.  And
...
method.  Now, you need to know, once you've request this object, you are
...
the rest of the body finalization.  So, once you've asked for manual control
you have to drive stick all the way home!
...
First lets that that closure and convert it into a class.  There's a few
...
Ok, now that we've written the class, how do we expose it to you L<Catalyst>
application.  I've always found that L<Catalyst::Model::Adaptor> is pretty
great at this.  Here's one way to adapt this class for L<Catalyst>
