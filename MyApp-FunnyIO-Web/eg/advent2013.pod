...
response to the filetest -s to do what we want. Well actually as subclassing
is probably not what you really want in most cases we are just going to do
a Moose delegation trick instead, which should work for us.
...
What we have at this point is a new class that is going to use some helpers
and wraps the important functions of L<IO:Uncompress::Gunzip> so we can work
with the handle later. Using Moose delegation via 'handles' we expose the
required methods from the underlying L<IO::Uncompress::Gunzip> object we
create so they appear native to the interface of this class. In this case
'_gunzip' is 'bare' because we won't actually need this accessor by itself,
just the delegated methods.
...
before. And now for a sensible contructor:
  ...
  around BUILDARGS => sub {
    ...
    return $class->$orig({
      '_gunzip'   => IO::Compress::Gunzip->new( $ref ),
      '_content'  => $ref
    });
  };
...
the object with a more Mooselike constructor, filling our accessors with
the L<IO::Uncompress::Gunzip> obect and the compressed content respectively.
...
L<IO::Uncompress::Gunzip> instead. So in our application config file:
